...
export function ResetPasswordForm({ token, error }: ResetPasswordFormProps) {
        ...
        return (
            <div className="space-y-5">
                ...
                <p className="text-center text-sm">
                    <Link href="/forgot-password" className="text-foreground font-medium hover:underline underline-offset-2 transition-colors">
                        Request a new link
                    </Link>
                </p>
            </div>
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (result.error) {
                toast.error(result.error.message || "Could not reset password. The link may have expired.");
                return;
            }
