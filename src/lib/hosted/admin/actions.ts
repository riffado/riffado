...
/**
 * Goodwill extension of the launch-window grace period for a single user.
 * Sets `users.planTransitionUntil` to `max(current, now() + days)`. Refuses
 * to move the boundary backwards (a shorter override would silently downgrade
 * an active grace user; explicit revoke is not modeled here -- it would need
 * its own action with stronger guardrails).
 */
export async function extendTransitionWindow(
    ctx: ActionContext,
    targetUserId: string,
    days: number,
): Promise<{ ok: true; planTransitionUntil: Date; extendedFrom: Date | null }> {
    assertReason(ctx.reason);
    ...
    return db.transaction(async (tx) => {
        ...
        const proposed = new Date(Date.now() + days * 86400 * 1000);
        const newUntil =
            u.planTransitionUntil && u.planTransitionUntil > proposed
                ? u.planTransitionUntil
                : proposed;
        if (
            u.planTransitionUntil &&
            newUntil.getTime() === u.planTransitionUntil.getTime()
        ) {
            await writeActionLog(tx, {
                ctx,
                action: "extend_transition_window_noop",
                targetUserId,
                targetResourceId: null,
                before: { planTransitionUntil: u.planTransitionUntil },
                after: { planTransitionUntil: u.planTransitionUntil },
            });
            return {
                ok: true,
                planTransitionUntil: u.planTransitionUntil,
                extendedFrom: u.planTransitionUntil,
            };
        }
        await tx
            .update(users)
            .set({ planTransitionUntil: newUntil })
            .where(eq(users.id, targetUserId));
        await writeActionLog(tx, {
            ctx,
            action: "extend_transition_window",
            targetUserId,
            targetResourceId: null,
            before: { planTransitionUntil: u.planTransitionUntil },
            after: { planTransitionUntil: newUntil, days },
        });
        return {
            ok: true,
