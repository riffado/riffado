...
- `POST /api/v1/recordings`: programmatic upload endpoint, the server-to-server counterpart of the browser `/api/recordings/upload`. Accepts `Bearer op_...` API keys (the browser path keeps cookie-only auth), takes multipart `file` plus optional `external_id` and `name`. `external_id` is a caller-supplied correlation handle that round-trips into every subsequent webhook (`recording.synced`, `transcription.completed`, etc.) so integrating systems can match events back to their own row without a side-table. Idempotent on `(user_id, external_id)` — a retry with the same `external_id` returns the existing row with HTTP 200 instead of inserting a duplicate or orphaning the already-uploaded blob. New partial unique index `recordings_user_id_external_id_unique` enforces this at the DB level. `external_id` is included in all v1 list/detail/webhook payloads (`null` for recordings created via Plaud sync or browser upload).
...
### Fixed
- Transcription in-flight claim release lacked an ownership check — a worker that stalled past `TRANSCRIPTION_STALE_TIMEOUT_MS` and then finally hit its `finally` block could clear the claim of a live successor that had legitimately taken over via the stale-claim path. The successor would then look unclaimed to a third caller, defeating the whole point of the atomic claim. Capture the claim timestamp at claim time and require `transcribing_started_at = claimedAt` in the release WHERE clause; if a successor has overwritten it, the release becomes a no-op and the live worker keeps its claim. Bug spotted by an automated review on the upstream PR.
- `/api/v1/*`: every request that hit an existing `api_rate_limit_buckets` row (i.e. the second hit from the same IP/identity, or any hit after the previous 60 s window had expired) failed with `ERR_INVALID_ARG_TYPE: The "string" argument must be of type string ... Received an instance of Date`, surfacing as HTTP 500 to the caller. Root cause: `consumeRateLimitBucket` interpolated `Date` values directly inside `sql<>` template literals — Drizzle's column-set path serializes `Date` correctly, but the SQL-template path falls through to `Date.prototype.toString()`, which postgres-js rejects. Pre-stringify to ISO so the binding is a plain string the driver accepts for timestamp columns. First-hit (INSERT) worked because `Date` in `.values({...})` is fine; the regression only triggered on the conflict-update path, which is why test suites with mocked-away DB never caught it.
- Manual `POST /api/recordings/[id]/transcribe` had no in-flight check, so every click spawned a fresh worker that raced against any previous click for the same recording. Browser reloads lost the "Transcribing audio…" indicator (it lived only in React state), so users naturally re-clicked and ended up with 3+ parallel workers all transcribing the same file and burning N× provider/CPU cost. Added a DB-level claim on `recordings.transcribing_started_at` taken inside an atomic UPDATE conditioned on `transcribing_started_at IS NULL OR < NOW() - 3h` — concurrent callers can't both pass because postgres serializes the writes, and the loser's UPDATE returns zero rows. The losing caller surfaces a new `TRANSCRIPTION_IN_PROGRESS` error code that the manual route maps to HTTP 409 (Conflict). The claim is released in a `finally` after success or failure; a crashed worker is recovered automatically when the next call sees its claim is older than the 3h stale timeout. v1 list/detail/webhook payloads gain a `transcription_in_progress` boolean derived from this state so the dashboard chip survives reloads and the UI no longer drifts into the rage-click pattern.
- Settings → Webhooks: the row action buttons (Deliveries / Edit / Delete) and the "Add Webhook" header action were unreachable in narrow Settings panes. The dialog clamps to `lg:max-w-[900px]` with `overflow-hidden`, leaving the main pane around 550–650 px wide after the nav sidebar — but the row used `sm:flex-row sm:justify-between` which triggers at 640 px **viewport** width, so on most desktop sizes the right-side action cluster got pushed off the visible edge with no scroll to recover it. Operators who tried to delete or recreate their first webhook (e.g. to rotate a lost signing secret) were stuck. Two changes: (1) stack the row content + actions vertically always, with `flex-wrap` covering mobile-portrait; (2) add a guaranteed-visible "Add Webhook" button at the foot of the list as a fallback for any case where the header's right-side action collides with the dialog's close button.
