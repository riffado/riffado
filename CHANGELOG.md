...
### Changed
- Hosted-only code moved under a `src/lib/hosted/` + `src/app/(hosted)/` directory boundary so the public/hosted split is enforced at directory level instead of guessed at per-file. Pure refactor with no behavior change: admin pages still serve at `/admin/*` and admin APIs still serve at `/api/admin/*` -- Next.js route groups (parentheses) don't affect URL paths. Self-host operators see no functional difference; existing admin bookmarks and API integrations continue to work.
- Landing pricing section simplified from three tiers (Self-host / Hosted Free / Hosted Pro) to two (Self-host / Hosted Pro). There is no hosted free plan; "pay or self-host" is the positioning. Hosted Pro pill changed from "Coming soon" to "14-day free trial" with real entitlement numbers (50 GB / 15 hr / unlimited devices). Founding-member offer surfaced in the pricing card.
- FAQ updated to reflect the two-tier model and trial pricing.
- Email verification is now required on hosted when SMTP is configured (`requireEmailVerification: true`). Self-host without SMTP keeps the historical no-verification path. Pre-existing users are grandfathered with `emailVerified = true` via the billing backfill script.

### Added
- Plaud directories ("filetags"): recordings can now be organized into the same flat, single-level directories the official Plaud app uses, with full two-way sync. Directories created, renamed, recolored, or deleted in Riffado are written through to Plaud first and appear identically in the official app (name, color, icon), and changes made in the official app are mirrored back on the next sync — including folder moves that don't bump the recording's version (reconciled without re-downloading audio). Dashboard gains a collapsible directory rail above the recording list (All / per-directory with icon+color+count / No directory), a create/edit dialog with the official 7-color palette and icon set, a "Move to directory" row action, and a directory chip in the player header. Manually uploaded recordings and users without a Plaud connection get local-only directories (marked "local"). New `plaud_filetags` table (names encrypted at rest) and `recordings.filetag_id` column (single migration); new endpoints `GET/POST /api/filetags`, `PATCH/DELETE /api/filetags/[id]`, and bulk `POST /api/recordings/filetag`, documented in `docs/API.md`. Deleting a directory never deletes recordings — they become unorganized.
- Full-data export archive ("Create full backup" in Settings → Export & Backup). Builds a zip per recording (`audio.<ext>` + `transcript.txt` + `summary.json`) plus a `manifest.json`, so leaving (self-host or hosted) means every recording, transcript, and AI summary round-trips in one archive -- not just metadata. Runs as an async job (`POST /api/backup` queues it, `GET /api/backup/[jobId]` polls status, `GET /api/backup/[jobId]/download` serves the result) processed by a new background worker (`src/lib/export/worker.ts`) instead of blocking the request: audio streams directly from storage into the zip and out to the destination, so building a large archive never buffers the whole thing in server memory. One active job per user, a 24-hour cooldown between backups, a per-tick concurrency cap, and a stale-job reclaim guard keep one user's export from starving others on shared hosted infrastructure. Audio is stored uncompressed in the zip (deflating already-compressed media wastes CPU for no size benefit); only the small text entries are deflated. A build is aborted on stalled progress -- no bytes written for 5 minutes -- rather than a fixed total-duration timeout, so large-but-healthy libraries aren't killed for taking a while; failed attempts retry up to 3 times before sticking as permanently failed. Completed archives are retained for 7 days and then swept by the same worker; an email is sent when the archive is ready. New `export_jobs` table and `StorageProvider.downloadStream`/`uploadStream`/`exists` methods (implemented for both local and S3-compatible storage). The lightweight `/api/export` (JSON/TXT/SRT/VTT, no audio) now also includes AI summaries in its JSON output.
- Admin "Exports" page (`/admin/exports`, hosted-only) for export-job visibility: pending/processing/retained-archive/failure tiles, total bytes currently retained across archives, and a filterable, paginated job list (status, recording count, size, timestamps, error message). Surfaces a warning banner when jobs sit in `processing` past the worker's own 30-minute stale-reclaim window, as an early signal the worker isn't running on some instance.
...
- Hosted Pro billing lifecycle (Stripe). Checkout (`POST /api/billing/checkout`), Customer Portal (`POST /api/billing/portal`), cancel (`POST /api/billing/cancel`), billing state (`GET /api/billing/me`), self-serve account deletion (`POST /api/billing/delete-now`). All routes 404 on self-host. Trial bootstrap stamps `plan = hosted_pro` + `planTransitionUntil = now + 14 days` on hosted signup via Better Auth database hooks.
- Founding-member pricing lock. Users who add a card within 6 months of `BILLING_LAUNCH_DATE` get `foundingMember = true` stamped on activation, locking their $5/mo rate for life.
- Grace period policy. Trial non-converts get 7 days before hard account deletion; paid customers who lapse get 30 days. Pre-launch users grandfathered into the 30-day (paid) grace path. `accountDeletionScheduledAt` on the users table drives the schedule; cleared on reactivation.
- Account deletion worker. Hard-deletes users whose `accountDeletionScheduledAt` has passed: best-effort R2 object cleanup, then `DELETE FROM users` cascading all FK-dependent rows. Confirmation email sent after deletion.
- Background sync for Hosted Pro users (`src/lib/hosted/sync/worker.ts`). Server-side 5-minute sync loop picks up Pro users whose last sync is stale (or never synced) and runs `syncRecordingsForUser` -- recordings arrive even when the browser is closed. No-ops on self-host. Landing pricing updated from "priority sync" to "background sync."
- Billing worker (`src/lib/hosted/billing/worker.ts`). 5-minute tick running: cycle-close, expired-trial demotion, due-deletion processing, grace-period reminder emails, grandfather-transition emails, and (every 6th tick) Stripe subscription reconciliation.
- Seven billing email templates: `welcome-hosted-pro`, `payment-failed`, `over-cap`, `grace-started`, `grace-reminder`, `grace-last-day`, `account-deleted`. All use first-write-wins dedup via the `email_log` table.
- `email_log` table for cross-process email dedup. `claimEmailSend(userId, eventKey)` returns true exactly once per key per user.
- `email-change-confirm` and `verify-email` transactional email templates wired into Better Auth's email verification and change-email flows.
- Settings billing section with plan/usage display, Stripe checkout + Customer Portal actions, and a grace-state banner (deletion countdown + export CTA + delete-now button).
- Dashboard trial banner for hosted users in their 14-day trial. Shows days remaining and founding-member "Add card" CTA. Dismissible via localStorage.
- Browser transcription support. `storeBrowserTranscription` server action + `POST /api/recordings/[id]/transcription/from-browser` route + "Transcribe in browser" button in the workstation detail pane.
- New env vars: `BILLING_ENABLED`, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_PRICE_ID_USD`, `STRIPE_PRICE_ID_EUR`, `BILLING_PRICE_USD`, `BILLING_PRICE_EUR`, `BILLING_DEFAULT_CURRENCY`, `BILLING_PRO_INTERVAL`, `BILLING_PRO_DESCRIPTION`, `BILLING_FREE_INCLUDED_SECONDS`, `BILLING_PRO_INCLUDED_SECONDS`, `BILLING_TRIAL_DAYS`, `BILLING_TRIAL_GRACE_DAYS`, `BILLING_PAID_GRACE_DAYS`, `BILLING_LAUNCH_DATE`.
- Schema: `user_plan` enum (`self_host`, `hosted_free`, `hosted_pro`); 7 new columns on `users` (`plan`, `planTransitionUntil`, `monthlyMynahSecondsRemaining`, `monthlyMynahGrantResetAt`, `foundingMember`, `everPaidAt`, `accountDeletionScheduledAt`); `email_log` table; partial unique index on `subscriptions(user_id)` for non-terminal rows.
- `ADMIN_HOSTNAME` env var (optional, unset by default). When set in production, a top-level `src/middleware.ts` gates `/admin/*`, `/api/admin/*`, and `/api/stripe/*` to that hostname (typically a dedicated subdomain like `admin.riffado.com`), and rejects customer-app paths on the admin host. Provides cookie / blast-radius isolation between the customer app and admin tooling without requiring a second deploy. Unset (default) keeps the admin surface on the same host as the customer app -- the correct behavior for self-host and dev. Boot-time validation refuses to start when `ADMIN_HOSTNAME` equals the `APP_URL` host, which would otherwise 404 every admin request.
- Email campaign primitives (schema + engine). New tables `email_campaigns`, `email_deliveries`, `email_suppressions`, `email_validations`, and `newsletter_subscriptions` ship to every install. The `sendCampaign` engine in `src/lib/email/` handles idempotent per-recipient delivery (the unique constraint on `(campaign_id, email)` makes campaigns safely resumable), suppression-list checking, marketing-consent gating for `marketing`-class kinds, optional Reacher-backed pre-flight validation, and RFC 8058 one-click unsubscribe headers. Dormant on self-host (no campaign content ships in the public repo today); useful infrastructure available to self-hosters who want to build their own transactional/announcement mail on top.
- Public newsletter signup at `POST /api/newsletter/subscribe` with double-opt-in via `GET /api/newsletter/confirm`. Per-IP rate limited (5/min). Bot honeypot. Confirmation email uses the existing SMTP transport. Stays a no-op when SMTP isn't configured (logs to console; client still gets 200 to avoid being a subscription-status oracle).
- Public stable unsubscribe endpoint at `GET|POST /api/email/unsubscribe`. HMAC-keyed tokens (no DB lookup table), constant-time verify, audience-separated (`u=<userId>` flips `users.marketing_email_consent`; `s=<subscriberId>` flips the newsletter row's `unsubscribed_at`). Lives on the customer host so links from old emails survive admin redeploys. RFC 8058 POST accepted for one-click compliance.
- `users.marketing_email_consent boolean default false` -- GDPR-safe opt-in for marketing-class email. Existing users default to false on migration. `announcement`-class campaigns (service notifications such as the upcoming rebrand) bypass this flag so material service changes always reach verified users.
- Capability interface at `src/lib/entitlements.ts`. `getEntitlements(userId)` returns the user's plan-derived capability set. Self-host always returns the permissive default (everything on); hosted reads from the `plan_entitlements` table written by the billing webhook handler. The seam every plan-gated feature should go through -- never read billing tables directly.
- Stripe billing integration (hosted-only). `STRIPE_SECRET_KEY` / `STRIPE_WEBHOOK_SECRET` / `STRIPE_PRICE_ID_USD` / `STRIPE_PRICE_ID_EUR` env vars; `subscriptions`, `billing_customers`, `plan_entitlements` tables; signature-verified webhook handler at `/api/stripe/webhook` (idempotent on `event.id`) dispatching `checkout.session.completed`, `customer.subscription.*`, `invoice.paid`, and `invoice.payment_failed`. Signup goes through Stripe Checkout (`mode: subscription`); self-service (update card, invoices, cancel) goes through the Customer Portal. Self-host returns 404 on the route (no Stripe wiring outside the Riffado-operated deployment).
- New optional env vars: `SMTP_MARKETING_FROM` (per-class From: override), `SMTP_REPLY_TO` (Reply-To applied to all outbound mail, independent of From:), `EMAIL_SEND_RATE_PER_SECOND` (campaign engine rate cap, default 5), `REACHER_API_URL` / `REACHER_API_KEY` (email-verification pre-flight; skipped entirely when key is unset).
- Public newsletter signup at `/updates`. Account-less audience for product updates -- self-host operators (who have no hosted account) and anyone who wants product news without signing up. Double-opt-in via the existing `/api/newsletter/{subscribe,confirm}` routes. Bot honeypot + per-IP rate limit on the public POST.
- Settings: "Product updates" toggle under Notifications (`marketingEmailConsent`). Default off (GDPR-safe opt-in). Distinct from the existing sync-notification email toggle; service-level announcements (rebrand, security advisories) ship regardless.
- One-time rebrand-announcement campaign (hosted-only). Template + audience definition under `src/lib/hosted/campaigns/`; maintainer scripts at `scripts/validate-rebrand-audience.ts` (Reacher pre-flight) and `scripts/send-rebrand-email.ts` (campaign run with `--dry-run`, `--limit`, `--only-email`, `--rate` flags, all `IS_HOSTED`-guarded with `--allow-self-host` override for dev). Class is `announcement` -- bypasses marketing-consent gating per the regulatory carve-out for material service changes, still honors the suppression list, still ships an unsubscribe footer.
- Stripe subscription mirror. The webhook and reconcile paths re-fetch the subscription from Stripe, normalize it (billing period from the subscription item, per the pinned `2026-06-24.dahlia` API version), upsert the `subscriptions` row, and derive `plan_entitlements` via `src/lib/hosted/billing/plans.ts`. Status-aware: only `active` / `trialing` / `past_due` grant Pro, and only when the Price id matches a configured Pro Price -- an unknown price yields free entitlements, never privilege escalation. Multi-currency (USD/EUR) resolved once at checkout from buyer geo and fixed for the subscription's life.
...
### Security
- Regression test (`src/tests/auth-cookie-scope.test.ts`) pins Better Auth session cookies to host-only. Fails the build if `src/lib/auth.ts` ever enables `crossSubDomainCookies` or adds a `domain:` cookie attribute, both of which would scope the customer-app session cookie to `.riffado.com` and leak it to the admin host once `ADMIN_HOSTNAME` is set.
