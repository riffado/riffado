...
- Founding-member monthly pricing (first-paid, first-served; the price stays locked for as long as the subscription remains active) and optional annual billing. Adds the `founding_member_reservations` table for atomic slot reservation/consumption tracking; `.env.example` documents optional `STRIPE_STANDARD_PRICE_ID_USD`/`_EUR` (post-founding-capacity monthly pricing), `STRIPE_PRICE_ID_USD_ANNUAL`/`_EUR_ANNUAL`, `BILLING_PRICE_USD_ANNUAL`/`_EUR_ANNUAL`, and `STRIPE_LEGACY_PRO_PRICE_IDS`. Inert on self-host unless `BILLING_ENABLED` is set ([#232](https://github.com/riffado/riffado/pull/232)).
