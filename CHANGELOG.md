...
- Adds the `founding_member_reservations` table and optional `.env.example` vars `STRIPE_STANDARD_PRICE_ID_USD`/`_EUR`, `STRIPE_PRICE_ID_USD_ANNUAL`/`_EUR_ANNUAL`, `BILLING_PRICE_USD_ANNUAL`/`_EUR_ANNUAL`, and `STRIPE_LEGACY_PRO_PRICE_IDS`. Inert on self-host unless `BILLING_ENABLED` is set ([#232](https://github.com/riffado/riffado/pull/232)).
