...
- Founding-member pricing lock. The first 100 monthly Pro subscribers can claim the configured founding monthly price ($5 USD by default, configured EUR equivalent). Checkout reserves a founding slot atomically before issuing a founding Stripe Price; consumed slots never reopen, and the benefit is forfeited when the subscription ends.
...
- New env vars: `BILLING_ENABLED`, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_PRICE_ID_USD`, `STRIPE_PRICE_ID_EUR`, `STRIPE_STANDARD_PRICE_ID_USD`, `STRIPE_STANDARD_PRICE_ID_EUR`, `STRIPE_PRICE_ID_USD_ANNUAL`, `STRIPE_PRICE_ID_EUR_ANNUAL`, `STRIPE_LEGACY_PRO_PRICE_IDS`, `BILLING_PRICE_USD`, `BILLING_PRICE_EUR`, `BILLING_STANDARD_PRICE_USD`, `BILLING_STANDARD_PRICE_EUR`, `BILLING_PRICE_USD_ANNUAL`, `BILLING_PRICE_EUR_ANNUAL`, `BILLING_FOUNDING_MEMBER_CAPACITY`, `BILLING_DEFAULT_CURRENCY`, `BILLING_PRO_INTERVAL`, `BILLING_PRO_DESCRIPTION`, `BILLING_FREE_INCLUDED_SECONDS`, `BILLING_PRO_INCLUDED_SECONDS`, `BILLING_TRIAL_DAYS`, `BILLING_TRIAL_GRACE_DAYS`, `BILLING_PAID_GRACE_DAYS`, `BILLING_LAUNCH_DATE`.
- Schema: `user_plan` enum (`self_host`, `hosted_free`, `hosted_pro`); 8 new columns on `users` (`plan`, `planTransitionUntil`, `monthlyMynahSecondsRemaining`, `monthlyMynahGrantResetAt`, `foundingMember`, `foundingMemberClaimedAt`, `everPaidAt`, `accountDeletionScheduledAt`); `founding_member_reservations`, `email_log` table; partial unique index on `subscriptions(user_id)` for non-terminal rows.
