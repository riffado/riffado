...
- **The one-line installer is part of the deploy surface.** `scripts/install.sh`, the `mesynx-ai.com/install.sh` route, and the version-pinned `mesynx-ai.com/vX.Y.Z/install.sh` route are a single contract — the script is served from the repo file via `src/app/install.sh/route.ts` and `src/app/[version]/install.sh/route.ts`. Breaking changes (renaming, removing prompts, changing the URL shape) need a deprecation cycle and a CHANGELOG note. Self-hosters paste these URLs into their own runbooks; don't break them silently.
