...
By default, Mesynx AI stores audio on the local filesystem (Docker
...
Mesynx AI talks to any service that implements the S3 wire protocol —
...
If MinIO runs in the same Docker network as Mesynx AI, the endpoint is
...
Mesynx AI's at-rest encryption applies to **database** fields, not audio
...
Mesynx AI does not ship a built-in "move all audio from local to S3"
