...
description: Every env var that Riffado reads, what it does, and what it defaults to.
---

Riffado validates every environment variable through a Zod schema in
...
| `APP_URL`                   | URL      | Public origin (e.g. `https://riffado.example.com`)                   |
...
| `RIFFADO_VERSION`                 | `latest`                           | Read by Docker Compose, not the app. Sets which image tag Compose pulls.                                            |
| `POSTGRES_PASSWORD`               | `postgres`                         | Read by Docker Compose, not the app. Password for the bundled `db` service. The one-line installer writes a random value. Changing this env var alone does not change an already-initialized database; rotate in place with `ALTER USER postgres WITH PASSWORD '…'`, write the same value to `.env`, and recreate the app. Recreating the `db` volume is not required. |
