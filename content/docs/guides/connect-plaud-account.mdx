...
description: Sign in to Plaud from Mesynx AI, including the Google/Apple workaround.
---

Mesynx AI signs into Plaud directly — the same way the official Plaud app
...
regional API host in `data.domains.api`, Mesynx AI follows the redirect
...
([#65](https://github.com/mesynx-ai/mesynx-ai/issues/65)).

Real "Sign in with Google / Apple" inside Mesynx AI is structurally
blocked by Google's authorized-origins policy on Plaud's OAuth client.
There are two workarounds.

### Easy path: the connector extension

Install the [Mesynx AI Connector](https://github.com/r0073d-l053r/mesynx/tree/main/chrome-extension)
browser extension (AGPL-3.0) — clone or download the repo and load it as an unpacked extension. With the extension installed, the
...
resulting access token to your Mesynx AI instance. No copy-pasting, no
devtools.
...
5. In Mesynx AI, switch to the **Paste token** tab, pick the region that
