...
Riffado exposes a small, stable automation surface for external
...
curl -H "Authorization: Bearer op_K7vR8x..." \
  https://your-riffado-host/api/v1/recordings
```

Internally, Riffado stores an HMAC-SHA256 hash of the key (keyed off
...
| `summary.completed`         | Auto-summary finished successfully             |
| `summary.failed`            | Auto-summary failed (transcript still kept)    |
...
| Header                  | Meaning                                                 |
| ----------------------- | ------------------------------------------------------- |
| `X-Riffado-Event`     | The event name (e.g. `recording.synced`)                |
| `X-Riffado-Delivery`  | Unique delivery id, useful for idempotent receivers     |
| `X-Riffado-Timestamp` | Unix seconds, also embedded in the signature payload    |
| `X-Riffado-Signature` | `t=<timestamp>,v1=<hex hmac>` — see below               |
...
at any host the Riffado container can reach. Docker-bridge service
...
Riffado instance is exposed to other people and you don't want it to
...
the Riffado UI or sync the source Plaud account.
