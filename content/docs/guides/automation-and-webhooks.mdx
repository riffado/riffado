...
Mesynx AI exposes a small, stable automation surface for external
...
curl -H "Authorization: Bearer op_K7vR8x..." \
  https://your-mesynx-ai-host/api/v1/recordings
```

Internally, Mesynx AI stores an HMAC-SHA256 hash of the key (keyed off
...
| Header                  | Meaning                                                 |
| ----------------------- | ------------------------------------------------------- |
| `X-MesynxAI-Event`     | The event name (e.g. `recording.synced`)                |
| `X-MesynxAI-Delivery`  | Unique delivery id, useful for idempotent receivers     |
| `X-MesynxAI-Timestamp` | Unix seconds, also embedded in the signature payload    |
| `X-MesynxAI-Signature` | `t=<timestamp>,v1=<hex hmac>` — see below               |
...
at any host the Mesynx AI container can reach. Docker-bridge service
...
Mesynx AI instance is exposed to other people and you don't want it to
...
the Mesynx AI UI or sync the source Plaud account.
