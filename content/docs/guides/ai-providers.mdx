...
- **Pick the cheap model.** The endpoint reuses the user's enhancement
  provider (or falls back to the transcription provider). A
  whisper-only id falls through to `gpt-4o-mini` / `llama-3.1-8b` for
  the chat call. Override the default model in Settings → AI providers
