...
The `/api/v1/*` surface is Mesynx AI's public, stable, read-only API.
...
`https://<your-mesynx-ai-host>/api/v1`
...
const res = await fetch(
  ...
  { headers: { Authorization: `Bearer ${process.env.MESYNX_AI_KEY}` } },
);
...
Mesynx AI UI or sync the source Plaud account via the configured
sync loop.
